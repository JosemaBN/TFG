...
import { applyMovementToStock, capOutNetToInventory, InsufficientStockError, } from "./services/stockService";
import { normalizeHora, parseEventDateFromBody, parseOptionalEventDate, } from "./utils/eventPayload";
    ...
    try {
        ...
        if (existing >= 1) {
            return res.status(409).json({
                error: "Solo puede existir un almacén; todo el inventario se gestiona ahí. No se pueden crear más.",
            });
        }
...
app.delete("/products/:id/movements", async (req, res) => {
    ...
    const eventId = typeof evRaw === "string" ? evRaw : Array.isArray(evRaw) ? evRaw[0] : undefined;
    try {
...
app.post("/events", async (req, res) => {
    const { name, code, poblacion, lugar, reference, notes, startDate, endDate, horaMontaje, horaPrueba, horaComienzo, horaFin, horaDesmontaje, } = req.body;
    if (!name) {
        return res.status(400).json({ error: "El nombre es obligatorio" });
    }
    try {
...
app.delete("/events/:id", async (req, res) => {
    const { id } = req.params;
    if (!id)
        return res.status(400).json({ error: "id de evento obligatorio" });
    try {
