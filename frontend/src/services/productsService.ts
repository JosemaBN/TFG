...
export function getProducts(options?: {
    ...
    if (options?.movementCounts)
        params.set("movementCounts", "true");
...
export function deleteProductMovements(productId: string, options?: {
    eventId?: string;
}): Promise<{
    deleted: number;
}> {
    ...
    return apiFetch<{
        deleted: number;
    }>(`/products/${encodeURIComponent(productId)}/movements${q}`, { method: "DELETE" });
}
