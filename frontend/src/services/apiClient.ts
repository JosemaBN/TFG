...
export const API_BASE_URL = (
  useDevProxy ? "" : (import.meta.env.VITE_API_URL ?? "http://127.0.0.1:3000")
).replace(/\/$/, "");
...
export async function apiFetch<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  try {
    res = await fetch(url, {
      ...init,
      headers,
      /** Evita respuestas GET en caché del navegador (plantilla OUT/IN deben reflejar la API al instante). */
