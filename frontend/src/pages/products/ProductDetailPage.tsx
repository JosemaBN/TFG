...
import { deleteProductMovements, getProductById, patchProductCompanyInventoryQty, updateProduct, } from "../../services/productsService";
...
export default function ProductDetailPage() {
    ...
    const outNet = useMemo(() => Math.min(outNetRaw, invForOutCap), [outNetRaw, invForOutCap]);
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            const wid = primaryWh != null && data.stock?.some((s) => s.warehouseId === primaryWh)
                ? primaryWh
    ...
    useEffect(() => {
        ...
        if (warehouseId != null) {
            const fromStock = product.stock?.filter((s) => s.warehouseId === warehouseId).reduce((sum, s) => sum + s.quantity, 0) ?? 0;
    ...
    async function handleClearAllMovements() {
        if (!id)
            return;
        if (!window.confirm("¿Eliminar todos los movimientos (OUT/IN) de este material en todos los eventos? La columna OUT en plantilla pasará a 0 (salvo que vuelvas a registrar salidas).")) {
            return;
        }
    ...
    return (<section>
      ...
      <PageHeader title={product?.name ?? "Detalle"} description="INV solo aquí; OUT y NAVE se calculan; REP se edita en la plantilla."/>
      {loading && <Spinner />}
      {error && <ErrorMessage message={error}/>}
      {!loading && !error && product && (<form onSubmit={handleSubmit} className="stack">
          <label>
            Área
            <input value={area} onChange={(e) => setArea(e.target.value)} disabled={saving} placeholder="Ej. Sonido, Iluminación"/>
          </label>
          <label>
            Tipo
            <input value={tipo} onChange={(e) => setTipo(e.target.value)} disabled={saving} placeholder="Ej. Mesa, Micro, Cable"/>
          </label>
          <label>
            Marca
            <input value={marca} onChange={(e) => setMarca(e.target.value)} disabled={saving} placeholder="Ej. Shure, Pioneer"/>
          </label>
          <label>
            Modelo *
            <input value={name} onChange={(e) => setName(e.target.value)} required disabled={saving} placeholder="Ej. SM58, CDJ-3000"/>
          </label>
          <label>
            INV — inventario total (empresa)
            <input type="number" min={0} step={1} value={inventoryQty} onChange={(e) => {
                const n = parseInt(e.target.value, 10);
                setInventoryQty(Number.isNaN(n) ? 0 : Math.max(0, n));
            }} disabled={saving || warehouseId == null}/>
          </label>
          {warehouseId == null ? (<p className="muted">No hay ningún almacén registrado; crea uno para fijar INV y línea de stock.</p>) : null}
          <div className="stack" style={{ gap: "0.35rem" }}>
            ...
            <button type="button" disabled={saving || clearingMovements || movements.length === 0} onClick={handleClearAllMovements}>
