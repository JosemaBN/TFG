...
function materialMarcaModelo(p: Product): string {
  ...
  const modelo = p.name;
  if (marca) return `${marca} ${modelo}`.trim();
  return modelo;
}
...
/**
 * Filas 1–4: Fecha, Población, Lugar, Evento.
 * Fila 5: MATERIAL | INV | OUT | REP | NAVE | Ud…
 * INV = inventario total (solo edición en materiales). OUT = neto OUT/IN en eventos. REP en plantilla.
 */
