...
export function PlantillaDescripcionCell(p: Props) {
    ...
    if (kind === "areaSection" && uid) {
        ...
        return (<span className="planilla-desc-with-toggle">
        <button type="button" className="planilla-section-toggle" onClick={() => ctx.toggleAreaCollapse(uid)} title={collapsed ? "Mostrar filas del área" : "Ocultar filas del área"}>
          {collapsed ? "▶" : "▼"}
        </button>
    ...
    if (kind === "tipoSection" && uid) {
        ...
        return (<span className="planilla-desc-with-toggle">
        <button type="button" className="planilla-section-toggle" onClick={() => ctx.toggleTipoCollapse(uid)} title={collapsed ? "Mostrar materiales del tipo" : "Ocultar materiales del tipo"}>
          {collapsed ? "▶" : "▼"}
        </button>
        <span className="planilla-desc-text">{v}</span>
      </span>);
