  ...
  const pinnedLeft: ColDef<PlanillaRow>[] = [
    ...
    },
    {
      ...
      valueGetter: (p) => {
        const d = p.data;
        if (!d) return "";
        if (d.rowKind !== "material" || !d.productId) return d.nave ?? "";
    ...
    children: [
      {
        ...
        cellClass: (p) => {
          ...
          if (q <= 0) return parts.join(" ");
          const mov = p.context?.getUdMovement?.(p.data.productId, ev.id);
          if (mov === "ENTRADA") parts.push("planilla-cell--mov-in");
          else if (mov === "SALIDA") parts.push("planilla-cell--mov-out");
          else parts.push("planilla-ud--positive");
          return parts.join(" ");
        },
      },
      {
...
export default function MaterialEventGrid() {
  ...
  /** Tras OUT OK: permite exactamente un IN hasta `clearInGate`. */
  ...
  const autoSizeAllToFit = useCallback((api: GridApi<PlanillaRow>) => {
    window.setTimeout(() => {
      /* Cabecera del grid a altura 0: no medir headers. */
      api.autoSizeAllColumns(true);
      /* INV/OUT/REP/NAVE: ceñir al contenido con el mínimo ancho posible. */
      api.autoSizeColumns({
  ...
  const toggleAreaCollapse = useCallback(
    (rowUid: string) => {
      setCollapsedAreaUids((prev) => {
        const n = new Set(prev);
        if (n.has(rowUid)) n.delete(rowUid);
        else n.add(rowUid);
        return n;
      });
      window.setTimeout(() => {
  ...
  const toggleTipoCollapse = useCallback(
    (rowUid: string) => {
  ...
  const pinnedTopRowData = useMemo(
    () => rowData.slice(0, PINNED_TOP_ROW_COUNT),
    [rowData]
  );
  const bodyRowData = useMemo(() => rowData.slice(PINNED_TOP_ROW_COUNT), [rowData]);
  const visibleBodyRows = useMemo(
    () => filterVisiblePlanillaRows(bodyRowData, collapsedAreaUids, collapsedTipoUids),
    [bodyRowData, collapsedAreaUids, collapsedTipoUids]
  );
  ...
  const onFirstDataRendered = useCallback(
    (e: FirstDataRenderedEvent<PlanillaRow>) => {
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      const api = gridApiRef.current;
      if (api && rowData.length > 0) autoSizeAllToFit(api);
    }, 100);
    return () => window.clearTimeout(t);
  }, [rowData, loading, autoSizeAllToFit]);

  const onCellValueChanged = useCallback(
    async (e: CellValueChangedEvent<PlanillaRow>) => {
      const field = e.colDef.field;
      if (field === "rep") {
        if (e.data?.rowKind !== "material" || !e.data.productId) return;
      ...
      if (!field?.startsWith("e_") || !field.endsWith("_ud")) return;
      if (e.data?.rowKind !== "material" || !e.data.productId) return;
      ...
      try {
        ...
        if (q === 0) {
          ...
          /** Servidor borra OUT/IN de ese evento×material; recargar filas para actualizar columna OUT. */
          const rows = await refreshGrid();
          if (rows === null) {
            window.alert(
              "Ud se guardó a 0 en el servidor, pero no se pudo recargar la plantilla. Recarga la página (F5) para ver OUT actualizado."
            );
          } else {
            const materialRow = rows.find(
              (r) => r.rowKind === "material" && r.productId === e.data?.productId
            );
            ...
            if (outAfter > 0) {
              window.alert(
                "Se eliminaron los movimientos de este evento para este material. OUT sigue siendo > 0 porque hay salidas/entradas en otros eventos (otras columnas). Pon Ud a 0 en cada columna donde hubiera movimientos, o en la ficha del material usa «Quitar todos los movimientos»."
              );
            }
          }
        } else {
  ...
  const getRowClass = useCallback((p: { data?: PlanillaRow }) => {
    const k = p.data?.rowKind ?? "material";
  ...
  const getRowId = useCallback((p: GetRowIdParams<PlanillaRow>) => {
    const d = p.data;
    if (d.rowUid) return d.rowUid;
    if (d.rowKind === "material" && d.productId) return `m-${d.productId}`;
    ...
    <div className="planilla-grid-wrap ag-theme-quartz">
      <AgGridReact<PlanillaRow>
        ...
        onGridReady={(e) => {
          ...
          if (rowData.length > 0) autoSizeAllToFit(e.api);
        }}
