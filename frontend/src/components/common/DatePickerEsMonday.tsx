...
export default function DatePickerEsMonday({ label, value, onChange, disabled }: DatePickerEsMondayProps) {
    ...
    return (<label>
      ...
      <div className="date-es">
        <input value={value} onChange={(e) => onChange(e.target.value)} placeholder="AAAA-MM-DD" inputMode="text" disabled={disabled} aria-label={label} onBlur={() => {
        ...
        <button type="button" className="date-es__btn" onClick={() => {
            const base = parseISODate(value) ?? new Date();
            setMonth(startOfMonth(base));
            setOpen(true);
        }} disabled={disabled} aria-label={`Elegir ${label}`} title="Elegir fecha">
          <svg width="18" height="18" viewBox="0 0 24 24" aria-hidden="true">
            <path fill="currentColor" d="M7 2h2v2h6V2h2v2h3a2 2 0 0 1 2 2v15a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h3V2Zm15 8H2v11h20V10ZM2 8h20V6H2v2Z"/>
          </svg>
        </button>
      </div>

      {open ? (<div className="date-es__modal-backdrop" role="presentation" onClick={() => setOpen(false)}>
          <div className="date-es__modal" role="dialog" aria-modal="true" aria-label={`Seleccionar ${label}`} onClick={(e) => e.stopPropagation()}>
            <div className="date-es__header">
              <button type="button" className="toolbar-button" onClick={() => setMonth((m) => addMonths(m, -1))} aria-label="Mes anterior">
                ←
              </button>
              <div className="date-es__title">{monthLabel}</div>
              <button type="button" className="toolbar-button" onClick={() => setMonth((m) => addMonths(m, 1))} aria-label="Mes siguiente">
                →
              </button>
            </div>

            <div className="date-es__grid">
              {["L", "M", "X", "J", "V", "S", "D"].map((d) => (<div key={d} className="date-es__dow">
                  {d}
                </div>))}
              {weeks.flat().map((cell) => {
                const isSelected = selectedIso && cell.iso === selectedIso;
                return (<button key={cell.iso} type="button" className={[
                        "date-es__day",
