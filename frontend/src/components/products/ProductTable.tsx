...
function sortKeys(keys: string[], label: (k: string) => string, emptyKey: string): string[] {
    return [...keys].sort((a, b) => {
        if (a === emptyKey && b !== emptyKey)
            return 1;
        if (b === emptyKey && a !== emptyKey)
            return -1;
...
export default function ProductTable({ products, onChanged }: ProductTableProps) {
    ...
    return (<div className="table-wrap">
      {areaMenuOpen || tipoMenuOpen ? (<div className="popover-backdrop" role="presentation" onClick={() => {
          ...
          <tr>
            <th>
              <span className="th-with-btn">
                <span>Área</span>
                <button ref={areaBtnRef} type="button" className="th-popover-btn" aria-haspopup="dialog" aria-expanded={areaMenuOpen} onClick={() => {
            setAreaMenuOpen((v) => !v);
            setTipoMenuOpen(false);
        }}>
                  ▾
              ...
              {areaMenuOpen ? (<div className="th-popover" role="dialog" aria-label="Filtrar por área">
                  <div className="th-popover-title">Áreas</div>
                  <div className="th-popover-list">
                    {areaKeys.map((k) => {
                const hidden = hiddenAreas.includes(k);
                return (<div key={k} className="th-popover-row">
                          <span className="th-popover-label">{labelArea(k)}</span>
                          <button type="button" className={hidden ? "toolbar-button" : "action-button"} onClick={() => toggleAreaHidden(k)}>
            ...
            <th>
              <span className="th-with-btn">
                <span>Tipo</span>
                <button ref={tipoBtnRef} type="button" className="th-popover-btn" aria-haspopup="dialog" aria-expanded={tipoMenuOpen} onClick={() => {
            setTipoMenuOpen((v) => !v);
            setAreaMenuOpen(false);
        }}>
                  ▾
              ...
              {tipoMenuOpen ? (<div className="th-popover" role="dialog" aria-label="Filtrar por tipo">
                ...
                return (<div key={k} className="th-popover-row">
                          ...
                          <button type="button" className={hidden ? "toolbar-button" : "action-button"} onClick={() => toggleTipoHidden(k)}>
                            {hidden ? "Mostrar" : "Ocultar"}
                          </button>
                        </div>);
            })}
            ...
            return (<tr key={p.id}>
              ...
              <td>
                <div className="table-inline-actions">
                  ...
                  <button type="button" className="table-action-danger" disabled={deletingId === p.id} onClick={() => void handleDelete(p.id, p.name)}>
